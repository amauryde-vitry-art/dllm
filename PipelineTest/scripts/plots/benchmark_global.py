...
def plot_mean_var_trajectories(ctx: PlotContext):
    ...
    ax.set_title("Mean Masked Entropy (averaged over samples)", fontweight="bold")
    ...
    ax.set_title("Variance of Masked Entropy Across Tokens (averaged over samples)", fontweight="bold")
    ax.set_xlabel("Diffusion Step")
...
def plot_scatter_features(ctx: PlotContext):
    ...
    ax.scatter(mean_ent[ctx.labels == 1], var_ent[ctx.labels == 1],
               alpha=0.5, color="red", s=20, label="Hallucination")
    ax.set_xlabel("Mean Masked Entropy")
    ax.set_ylabel("Var Masked Entropy Across Tokens")
    ax.set_title("Scatter: MeanMaskedEntropy vs VarMaskedEntropyAcrossTokens", fontweight="bold")
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
...
def plot_scatter_all_features(ctx: PlotContext):
    ...
    for i in range(len(top_k)):
        for j in range(i + 1, len(top_k)):
            ...
            ax.scatter(features[ctx.labels == 0, fi], features[ctx.labels == 0, fj],
                       alpha=0.4, color="green", s=15, label="Correct")
            ax.scatter(features[ctx.labels == 1, fi], features[ctx.labels == 1, fj],
                       alpha=0.4, color="red", s=15, label="Halluc")
            ax.set_xlabel(names[fi], fontsize=8)
            ax.set_ylabel(names[fj], fontsize=8)
    ...
    plt.suptitle("Top Feature Pairs (by AUC)", fontweight="bold")
    plt.tight_layout()
