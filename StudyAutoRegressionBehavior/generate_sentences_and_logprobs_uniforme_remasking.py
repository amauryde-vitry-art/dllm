    ...
    for num_block in range(num_blocks):
        ...
        for i in range(steps_per_block):
            ...
            for j in range(batch_size):
                current_block_indices = torch.where(init_block_mask[j])[0]
                num_indices = len(current_block_indices)
                k = i
                current_block_indices = torch.where(init_block_mask[j])[0]
                shuffled_indices = current_block_indices[torch.randperm(len(current_block_indices))]
                indices_to_remask = shuffled_indices[k:]
                x[j, indices_to_remask] = mask_id
            ...
            for j in range(batch_size):               
                ...
                current_world_list = [tokenizer.decode(t).strip() for t in x[j][init_block_mask[j]]] # type: ignore
                levenshtein_distances = [distance(word1, word2) for word1, word2 in zip(words_list, words_list_history)]

                tokens_maks = x_masked[j][init_block_mask[j]]
                text_masked_list = [tokenizer.decode(t).strip() for t in tokens_maks]
                start_idx = block_start[j].item()
                end_idx = block_end[j].item()


                res_list[j][num_block][i] = (current_world_list, current_block_probs, block_mask[j][start_idx:end_idx], changes[j][start_idx:end_idx], semantic_distance_j, levenshtein_distances, text_masked_list)
...
def concat_text_masked(res):
    final_text_masked = []
    nb_iterations = len(res[0].keys())
    for i in range(nb_iterations):
        text_masked_list = []
        for key in res.keys():
            text_masked = res[key][i][6]
            text_masked_list.append(text_masked)
        final_text_masked.append(np.concatenate(text_masked_list, axis=0))
    return final_text_masked
